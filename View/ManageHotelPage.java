...
public class ManageHotelPage implements ActionListener{
    ...
    public void launchManageHotelPage (JFrame mainFrame) {
        ...
        chnButton.addActionListener(e -> {
            ChangeHotelNamePage chnp = new ChangeHotelNamePage();
             chnp.launchChangeHotelNamePage(mainFrame);
            });

        rrButton.addActionListener(e -> { 
            RemoveRoomPage rrp = new RemoveRoomPage();
            rrp.launchRemoveRoomPage(mainFrame);
            });

        resButton.addActionListener(e -> {
            RemoveReservationPage rres = new RemoveReservationPage();
            rres.launchRemoveReservationPage(mainFrame);
            });

        arButton.addActionListener(e -> {
            AddRoomPage arp = new AddRoomPage();
            arp.launchAddRoomPage(mainFrame);
            });

        ubButton.addActionListener(e -> {
            UpdateBasePricePage ubp = new UpdateBasePricePage();
            ubp.launchUpdateBasePricePage(mainFrame);
            });

        rhButton.addActionListener(e -> {
            RemoveHotelPage rhp = new RemoveHotelPage();
            rhp.launchRemoveHotelPage(mainFrame);
            });
